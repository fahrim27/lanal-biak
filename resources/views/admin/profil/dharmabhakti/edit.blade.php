...
<div class="card" id="card-admin" style="margin-right: 2%; width: 100%; size: 100px; font-size: 25px;">
   ...
   <div class="card-body">
      ...
      @endif
      <form method="POST" enctype="multipart/form-data" action="{{route('admin.profil.dharmabhakti.update', $viewData["data"]->id)}}" style="margin-right: 20px; font-size: 25px;">
         @csrf
         @method('put')
         <div class="form-group">
            <label for="title" class="form_label">Judul</label>
            <input type="text" name="title" class="form-control" placeholder="" value="{{$viewData["data"]->title}}" required />
         </div>

         <div class="form-group">
            <label for="image" class="form_label">Image <a href="{{url('storage/images/profil/dharmabhakti/'.$viewData["data"]->image)}}" target="__blank"><i class="fa fa-info-circle"></i></a></label>
