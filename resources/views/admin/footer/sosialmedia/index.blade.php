...
<div class="content-header row">
    <div class="content-header-left col-md-12 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title mb-0">Data Sosial Media</h2>
                                        ...
                                        @foreach ($viewData["data"] as $value)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <form id="delete-form-{{$value->id}}" action="{{ route('admin.footer.sosialmedia.destroy', $value->id)}}" method="post">
                                                      @csrf
                                                      @method('delete')
                                                    </form>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Create Sosial Media</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form method="POST" enctype="multipart/form-data" action="{{ route('admin.footer.sosialmedia.store')}}">
                    @csrf
    ...
    @foreach ($viewData["data"] as $value)
        <div class="modal fade" id="update-modal-{{$value->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Update Sosial Media #{{$value->id}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form method="POST" enctype="multipart/form-data" action="{{ route('admin.footer.sosialmedia.update', $value->id)}}">
                    @csrf
                    @method('patch')
                  ...
                  <div class="form-group">
                    <label for="icon" class="col-form-label">Icon:</label>
                    <input id="icon" class="form-control" type="text" name="icon" value="{{$value->icon}}" required>
                  </div>
...
@section('js')
    ...
    <script>
        $("#delete-btn-{{$value->id}}").on('click', function (e) {
            e.preventDefault();
            Swal.fire({
                title: 'Delete Sosial Media',
                text: "Are You Sure Want Delete This Data, Sosial Media {{$value->title}} ?",
                icon: 'warning',
